...
export function SearchView(){
  ...
  return <><header class="page-head"><div><div class="eyebrow">Global index</div><h1>Search</h1><p>Full-text search across documents, headings, tasks, and bug rows.</p></div></header><input class="search-trigger" style={{width:"100%",maxWidth:800}} autofocus value={query} onInput={(event)=>setQuery(event.currentTarget.value)} placeholder="BUD-3 · m:Budget is:open · lane:Now e:S · t:bug s:blocker"/>{!searchReady.value?<div class="empty" style={{marginTop:20}}>Building the local search index…</div>:!results.length?<EmptyState icon="search" title={query?"No results":"Search the PM vault"}>Plain text, or filters: <code>m:</code> campaign · <code>lane:</code> Now/Next/Later · <code>s:</code> severity · <code>e:</code> effort · <code>is:</code> open/done · <code>id:</code> chip · <code>t:</code> type · <code>f:</code> file. Results are permalinkable.</EmptyState>:<div class="search-list" style={{marginTop:20}}>{results.map((result)=><a class="search-hit" href={href(result)} style={{color:"inherit"}}><div style={{display:"flex",gap:8}}><Chip>{result.type}</Chip>{result.idChip&&<Chip tone="id">{result.idChip}</Chip>}<strong>{result.title||result.text}</strong></div><div class="muted" style={{fontSize:11,marginTop:7}}>{result.module} · {result.relPath}</div></a>)}</div>}</>;
}

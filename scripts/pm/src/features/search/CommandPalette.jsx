...
const actions=[
  ...
  {label:"Open search",href:"#/search",icon:"search"},{label:"Open idea inbox",href:"#/inbox",icon:"inbox"},
];
...
export function CommandPalette(){
  ...
  return <div class="palette-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&(paletteOpen.value=false)}><section class="palette" role="dialog" aria-modal="true" aria-label="Command palette"><div class="palette-input"><Icon name="search"/><input ref={input} value={query} onInput={(event)=>{setQuery(event.currentTarget.value);setSelected(0);}} onKeyDown={key} placeholder="Search docs, tasks, bugs, or run a command…"/>{!searchReady.value&&<span class="chip">indexing…</span>}</div><div class="palette-results">{!results.length?<div class="empty">No results. Try m:Budget, s:blocker, lane:Now, e:S, or id:BUD-3.</div>:results.map((row,index)=><button class={`palette-result ${index===selected?"selected":""}`} onMouseEnter={()=>setSelected(index)} onClick={()=>choose(row)}><Icon name={row.icon||"search"}/><span class="palette-result-main"><strong>{row.label}</strong>{row.detail&&<span>{row.detail}</span>}</span></button>)}</div><div class="palette-footer">↑↓ navigate · Enter open · filters: m: t: s: is: f: lane: e: id:</div></section></div>;
}

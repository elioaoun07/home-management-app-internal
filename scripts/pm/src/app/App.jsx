...
import { files, hideCompleted, loadError, loading, menuOpen, modal, offlineSnapshot, paletteOpen, theme } from "./store.js";
...
export function App(){
  ...
  return <div class={`app-shell ${menuOpen.value?"menu-open":""}`}><Sidebar/><div class="main-column"><header class="topbar"><button class="icon-button mobile-menu" onClick={()=>{menuOpen.value=!menuOpen.value}}><Icon name="menu"/></button><button class="search-trigger" onClick={()=>{paletteOpen.value=true}}><Icon name="search"/><span>Search or run a command</span><kbd>Ctrl K</kbd></button><div class="topbar-spacer"/><button class={`icon-button ${hideCompleted.value?"is-on":""}`} aria-pressed={hideCompleted.value} onClick={()=>{hideCompleted.value=!hideCompleted.value}} title={hideCompleted.value?"Showing open items only — click to show completed":"Hide completed items"}><Icon name={hideCompleted.value?"eye-off":"eye"}/></button><button class="theme-button" onClick={()=>{theme.value=theme.value==="auto"?"blue":theme.value==="blue"?"frost":"auto"}} title={`Theme: ${theme.value}`}><Icon name="sun"/></button>{globalThis.PM_MODE==="server"&&<button class="icon-button" onClick={()=>{modal.value={type:"idea"}}} title="Capture idea"><Icon name="bulb"/></button>}{globalThis.PM_MODE==="server"&&<button class="icon-button" onClick={()=>{modal.value={type:"create",dir:""}}} title="Create document"><Icon name="plus"/></button>}</header>{offlineSnapshot.value&&<div class="offline-banner"><Icon name="clock"/>Offline snapshot from {new Date(offlineSnapshot.value.cachedAt).toLocaleString()} — reconnect to your laptop's Wi-Fi for live data.</div>}<main class="content"><RouterOutlet/></main><MobileNav/></div>{menuOpen.value&&<button aria-label="Close navigation" onClick={()=>{menuOpen.value=false}} style={{position:"fixed",zIndex:55,inset:0,background:"rgba(0,0,0,.45)"}}/>}<CommandPalette/><SourcePreview/><FileOpsModals/><ToastHost/></div>;
}

function RouterOutlet(){const current=route.value;if(current.name==="home")return <HomeView/>;if(current.name==="module")return <ModuleView/>;if(current.name==="doc")return <DocView/>;if(current.name==="tasks")return <TasksView/>;if(current.name==="checklist")return <ChecklistRollup/>;if(current.name==="bugs")return <BugsRollup/>;if(current.name==="search")return <SearchView/>;if(current.name==="inbox")return <InboxView/>;if(current.name==="delivery"&&globalThis.PM_MODE==="server")return <DeliveryHome/>;if(current.name==="delivery-new"&&globalThis.PM_MODE==="server")return <DeliveryWizardPage/>;if(current.name==="delivery-session"&&globalThis.PM_MODE==="server")return <SessionDetail/>;if(["delivery","delivery-new","delivery-session"].includes(current.name)&&globalThis.PM_MODE!=="server"){location.hash="/";return null;}return <EmptyState title="Page not found"><a href="#/">Return to the command center</a></EmptyState>;}

...
export default function TransactionSizeDistributionWidget({ transactions }: Props) {
  const [mode, setMode] = useState<ViewMode>("count");

  const { chartData, median, mean, totalCount } = useMemo(() => {
    if (!transactions.length) return { chartData: [], median: 0, mean: 0, totalCount: 0 };
  ...
  if (!transactions.length || totalCount < 5) {
    return (
      <WidgetCard title="Transaction Distribution">
        <p className="text-white/40 text-xs text-center py-8">Need at least 5 transactions</p>
      </WidgetCard>
    );
  }
  ...
  const medianBucket = BUCKETS.findIndex((b) => median >= b.min && median < b.max);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        <div className="neo-card rounded-lg p-2 text-center">
          <p className="text-[9px] text-white/40">Median</p>
          <p className="text-sm font-bold text-amber-400">${Math.round(median)}</p>
        </div>
        <div className="neo-card rounded-lg p-2 text-center">
          <p className="text-[9px] text-white/40">Average</p>
          <p className="text-sm font-bold text-cyan-400">${Math.round(mean)}</p>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
            ...
            <YAxis
              ...
              axisLine={false}
              tickLine={false}
              tickFormatter={(v) => mode === "total" && v >= 1000 ? `$${(v / 1000).toFixed(0)}k` : String(v)}
              width={mode === "total" ? 38 : 25}
            />
            <Tooltip
                ...
                return (
                  <div className="neo-card rounded-lg px-3 py-2 text-xs border border-white/10">
                    ...
                    <p className="text-white font-bold">
                      {mode === "count" ? `${d.count} transactions` : `$${d.total.toLocaleString()} total`}
                    </p>
                    {mode === "count" && (
                      <p className="text-white/40">${d.total.toLocaleString()} total</p>
                    )}
                  </div>
                );
              }}
            ...
            {medianBucket >= 0 && (
              <ReferenceLine
                ...
                strokeWidth={1.5}
                strokeDasharray="4 2"
                label={{ value: "med", fill: "#fbbf24", fontSize: 8, position: "top" }}
              />
            )}
            {meanBucket >= 0 && meanBucket !== medianBucket && (
              <ReferenceLine
                ...
                strokeWidth={1.5}
                strokeDasharray="4 2"
                label={{ value: "avg", fill: "#22d3ee", fontSize: 8, position: "top" }}
              />
            )}
          </BarChart>
        </ResponsiveContainer>
      ...
      {/* Insight line */}
      {(() => {
        const smallTxns = chartData.slice(0, 3).reduce((s, d) => s + d.count, 0);
        const largeTxns = chartData.slice(4).reduce((s, d) => s + d.count, 0);
        const smallPct = totalCount > 0 ? Math.round((smallTxns / totalCount) * 100) : 0;
        const largePct = totalCount > 0 ? Math.round((largeTxns / totalCount) * 100) : 0;
        return (
          <p className="text-[10px] text-white/30 text-center mt-2">
            {smallPct}% of transactions under $50 · {largePct}% over $250
          </p>
        );
      })()}
    </WidgetCard>

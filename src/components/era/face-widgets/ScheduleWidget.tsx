...
export function ScheduleWidget({ variant }: { variant: Variant }) {
  ...
  const { todayCount, overdueCount } = data;
  const stat = todayCount === 0 ? "Clear today" : `${todayCount} due today`;
  const body =
    overdueCount > 0
      ? `${overdueCount} overdue`
      : todayCount === 0
      ? "Nothing urgent"
      : data.firstTitle ?? "Stay on track";

  return <WidgetContent variant={variant} stat={stat} body={body} hue={256} />;
}

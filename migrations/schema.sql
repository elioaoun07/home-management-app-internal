...
CREATE TABLE public.trips (
  ...
  scope text NOT NULL DEFAULT 'household'::text CHECK (scope = ANY (ARRAY['solo'::text, 'household'::text])),
  status text NOT NULL DEFAULT 'draft'::text CHECK (status = ANY (ARRAY['draft'::text, 'upcoming'::text, 'active'::text, 'completed'::text, 'archived'::text])),
  start_date date,
  end_date date,
  account_id uuid,
...
CREATE TABLE public.trip_places (
  ...
  name text NOT NULL,
  place_type text CHECK (place_type = ANY (ARRAY['hotel'::text, 'activity'::text, 'restaurant'::text, 'attraction'::text, 'transport'::text, 'note'::text, 'other'::text])),
  url text,
  description text,
  cost numeric,
  currency text DEFAULT 'USD'::text,
  priority text NOT NULL DEFAULT 'flexible'::text CHECK (priority = ANY (ARRAY['mandatory'::text, 'flexible'::text, 'wishlist'::text])),
  scheduled_date date,
...
CREATE TABLE public.trip_side_effects (
  ...
  user_id uuid NOT NULL,
  trip_id uuid NOT NULL,
  effect_type text NOT NULL CHECK (effect_type = ANY (ARRAY['chore_skip'::text, 'event_skip'::text, 'recurrence_pause'::text, 'meal_skip'::text, 'item_reassign'::text])),

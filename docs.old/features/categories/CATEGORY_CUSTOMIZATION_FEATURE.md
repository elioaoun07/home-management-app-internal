...
> **Module:** `src/features/categories/` | **API:** `src/app/api/categories/`
> **DB Tables:** `user_categories`
> **Status:** Active

## Overview

Users can fully customize categories and subcategories in Settings → Categories tab: reorder (drag-and-drop), add/remove, change color, toggle visibility. Each category is mapped to a futuristic SVG icon automatically based on its name/slug.
...
Categories are automatically mapped to futuristic SVG icons based on their name/slug — no manual assignment needed.

**Utility:** `src/lib/utils/getCategoryIcon.tsx`
- `getCategoryIcon(name, slug)` → returns SVG icon component
- `getCategoryGlowClass(color)` → returns Tailwind drop-shadow class

**Matching logic:** subcategory keywords first (e.g., "coffee", "fuel"), then parent keywords (e.g., "food", "transport"), fallback to `DollarSignIcon`.

**Icon sizes:**
- Category grid (expense form): `w-8 h-8`
- Transaction lists: `w-5 h-5`
- Category header: `w-12 h-12`
- Stats cards: `w-6 h-6`

**Adding a new icon:**
1. Create SVG component in `src/components/icons/FuturisticIcons.tsx` (24×24 viewBox, 2px stroke)
2. Add keyword match in `getCategoryIcon`
3. Import the new icon in `getCategoryIcon`

**Neon glow colors** (applied via `getCategoryGlowClass`):
- Default cyan: `drop-shadow-[0_0_8px_rgba(6,182,212,0.6)]`
- Color variants: green, blue, yellow, red, orange, purple, pink

**Note:** Old emoji values in the database are ignored — the system derives icons from name/slug only. No DB migration needed.

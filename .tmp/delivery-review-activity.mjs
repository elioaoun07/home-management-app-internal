...
try {
 ...
 store=openStore({path:join(root,'fixture.sqlite')});let sdkCalls=0;let unblock;const hold=new Promise(resolve=>{unblock=resolve;});
 const runtime={kind:'fake',workspaceFor:({access})=>({root:'/work',backing:'fake',access}),binding:async(backend_id)=>({backend_id,sdk_version:'fixture',boundary_digest:'sha256:fake',battery_digest:'sha256:fake'}),provision:async()=>({base_manifest:[],refusals:[],importSdk:async()=>({query:({options})=>(async function*(){sdkCalls++;yield {type:'system',subtype:'init',model:'claude-test',session_id:options.resume||options.sessionId};yield {type:'assistant',parent_tool_use_id:null,message:{content:[{type:'tool_use',name:'Read'}]}};await hold;const result=JSON.stringify({outcome:'emit 20',scope:['src/a.ts'],steps:['edit'],risks:[],unknowns:[],checks:[],questions:[]});yield {type:'result',subtype:'success',is_error:false,result,total_cost_usd:0.01,usage:{input_tokens:1,output_tokens:1}};})()})}),release:async()=>{},recover:async()=>null};
 ...
 const first=createJourney(args), result=await first.deliver({file:'Budget/4 - Checklist.md',cbidx:0,expectLine:row,expectId:'BUD-14',executor:'claude',model:'claude-test',effort:'low',command_id:'fixture-first',actor:'owner'});
 await new Promise(resolve=>setTimeout(resolve,100));
 console.log('during streaming after Read event',JSON.stringify({jobs:store.listJobs(result.run_id).map(j=>({status:j.status})),activity:first.detail(result.run_id).activity,sdkCalls}));
 unblock();await first.idle();
 console.log('after stream ends',JSON.stringify({jobs:store.listJobs(result.run_id).map(j=>({status:j.status})),activity:first.detail(result.run_id).activity,sdkCalls}));

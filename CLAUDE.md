...
15. **Floating panels (dropdowns, popovers, command palettes) must be opaque** — never `neo-card` (semi-transparent glass) on a panel floating above page content: glass/blur causes text bleed-through from what's behind. Use `tc.bgPage` from `useThemeClasses()` so the panel is the same solid color as the page background per theme. `neo-card` is for non-overlaid cards only.
16. **Fixed/sticky headers must not overlap page content** — `fixed`/`sticky` headers (`h-14`, etc.) **must** have matching top padding on the content below (e.g. `pt-14`). On standalone/isolated pages (NFC, guest portal, …) that render their own layout, also make the root layout's `ConditionalHeader` and `MobileNav` hide on those routes — otherwise a fixed header with no content offset overlaps. Always verify on mobile viewport.
...
26. **The Supabase MCP connection is READ-ONLY and exists ONLY for the Delivery Command Center bridge** — the live project holds the owner's **real production data** (money, schedule, household). No agent may write to it, ever, by any route. Absolute: **not waivable by an in-session instruction, a plan, a skill, a subagent prompt, or "just this once"** — only the owner editing this rule waives it.

   - **Permitted scope:** read-only inspection serving the **Delivery Command Center / PM mobile relay bridge** (`pm_live`, `pm_commands` — `migrations/2026-07-25_pm-mobile-relay.sql`, `scripts/pm/bridge.mjs`). Nothing else, including "while I'm here" cleanups.
   ...
   - **Forbidden — never call, never propose calling:** `execute_sql` (even a `SELECT`), `apply_migration`, `deploy_edge_function`, any `*_branch` (create/delete/merge/reset/rebase), `create_project`, `pause_project`, `restore_project`, `confirm_cost`. Blocked via `deny` in `.claude/settings.json` + `~/.claude/settings.json`; a `deny` cannot be overridden by approving a prompt.
   - **No equivalent back doors.** The ban is on the *effect*, not the tool name. Do not reach the production DB via the Supabase CLI (`supabase db push/reset/execute`), `psql`, a Node/`tsx` script or REPL using `SUPABASE_SERVICE_ROLE_KEY` / `supabaseAdmin()`, `curl` against PostgREST or RPC endpoints, a dev-only API route that runs SQL, or a cron/seed/repair script run for its side effect. Writing such a script **and running it** is the same violation as calling `execute_sql`.
   - **DB changes stay manual, as #24 already requires:** write `migrations/YYYY-MM-DD_*.sql`, update `schema.sql`, then **stop and hand the SQL to the owner** for the Supabase SQL Editor. Never "just apply it", and never report a migration as applied — only the owner can confirm that.
   - **`data-repair` runbooks are deliverables, not actions.** Produce the inspect/backup/fix/verify SQL for the owner; the agent never executes it.
   - **If a task appears to require a DB write:** stop, state exactly what write is needed and why, and hand it over. A blocked task reported honestly is correct; a silently-executed write is a breach.
...
27. **Visibility / permission bugs: ask for the DB's RLS state BEFORE reading application code.** *(Added 2026-08-04 after this exact loop cost five turns and a false root cause.)* When the symptom is shaped like **"X can't see Y"** — partner/household can't see a shared record, a role sees an empty list, data appears for one user but not another, a list is silently empty with no error — the filter is at least as likely to be in **RLS** as in the route. RLS is invisible to this repo, so reading route code cannot rule it out, and correct-looking code is *not* evidence the code is fine.
   ...
   - **First action, before Grep/Read on any route: read `migrations/db-state.json`** — the committed snapshot of live DB state: RLS enablement, full policy bodies (`using`/`with_check`/`permissive`), FK `on_delete` cascades, triggers, SECURITY DEFINER function bodies. Generated by the owner running `migrations/db-state.sql`; validate with `pnpm db:verify-rls`. It is the **only** repo artifact permitted as evidence about RLS. If it's missing or stale the SessionStart hook says so — believe the hook, not the prose.
   - **If the snapshot is missing or stale**, hand the owner this and wait for the output. Two queries, and the guessing ends:
     ```sql
     ...
     ```
     Ask for it **untruncated** — a cut-off `qual`/`permissive` column is how you ship a permissive policy that a restrictive one overrides.
   - **No repo artifact is evidence about RLS.** Not `schema.sql` (tables-only), not a vault Overview, not a migration comment, not a Master Book entry, not this file. A doc claim about DB state is a **hypothesis to verify**, never a fact to reason from. In the 2026-08-04 Trips bug three separate documents asserted "no RLS on the trips family"; all three were wrong, and the claim was load-bearing — a new table had been shipped policy-free *because* of it.
   - **Silent-empty is the signature.** RLS never errors, it removes rows: "correct query + zero rows + no error + works for the owner but not the partner" is the fingerprint. And if a SECURITY DEFINER RPC path works while the equivalent PostgREST path doesn't, that's near-proof of RLS (SECURITY DEFINER bypasses it). Check that disagreement *early* — cheapest discriminator in the codebase.
   - **Don't "fix" correct code to chase a symptom.** If every layer you read is right, say so and go get DB state; don't rewrite working routes on a hunch.
   - **When adding any child table**, give it a policy in the same migration, or state in the migration why it needs none. "The parent has no RLS" is not a reason — verify the parent.

28. **UI text is a cost, not a feature — write the minimum and stop.** *(Added 2026-08-24 after the owner called this out: "you are adding on production, on the UI, way too many texts to explain things that are pretty clear for me… it is getting too frustrating.")* The reflex to explain a control in the control is wrong here: this is a **single-household app whose only users built it**. They know what a button does, and every sentence of rationale steals vertical space on a phone and makes a simple screen read as complicated.

   - **Default to no explanatory prose.** A label, a number, an action verb. If a card reads from its title + amount + buttons, it needs nothing else.
   ...
   - **No banner that restates the tab it sits under.** An "Imported" tab needs no line explaining that these rows were imported.
   - **One short clause is the ceiling** where genuinely ambiguous — e.g. `Also in Debit Card - NEO`. Never a sentence, never a rhetorical question.
   ...
   - **When help is genuinely needed, use an `i` affordance** (tap/hover popover), not inline copy — available, invisible by default.

...
# Performance · FABLED 3.5 — Successor Briefing
...
You are an AI model about to edit documents in `ERA Notes/05 - Performance/`. Docs work is the safest work in the repo — but these files are load-bearing (agents boot from them), so wrong docs are worse than no docs.

## First 5 minutes

```bash
git log --format="%h %ad %s" --date=short --since=2026-07-18 -- "ERA Notes/05 - Performance"
```
...
- Reading the RPC/bundle doctrine here is mandatory before touching hot paths; writing perf docs is any-model.
...
| Trap | Symptom | Guard |
|---|---|---|
| Hot paths are RPC-only | adding per-child fetches to schedule/health pages | Hard Rules 20/21; `get_schedule_bundle` / `get_health_bundle` are the pattern |

## Verification manifest

| Claim | Command | Expected |
|---|---|---|
| Zero-commit claim | `git log --oneline c561635..f0a8e19 -- "ERA Notes/05 - Performance" | wc -l` | 0 |

## What FABLED 2 got wrong here

Nothing — nothing moved to be wrong about.

...
# Setup & Onboarding · FABLED 3 — Index
...
|---|---|---|---|
| **Overall** | **5.3** | = | Excellent daily steering; fragile disaster story. *(carried — per-dimension detail in the [frozen v2 scoreboard](<../FABLED 2/_index.md>))* |
| **Handoff readiness** | **7** | new | Env/setup doc edits are any-model; keep docs/ENV.md and vault setup docs agreeing. |

## Delta ledger (FABLED 3 era, append-only)

- **2026-07-18** (generation created):
  - One commit touched the section; `docs/ENV.md` (repo-side) gained GOOGLE_CLIENT_ID/SECRET/REDIRECT_URI for gcal (`9d867f8`) — vault setup docs did not mirror it.

## The next moves

1. Mirror the gcal env vars into the vault setup doc (or point at docs/ENV.md as single source).
2. Carry v2's disaster-recovery ask: the backup/restore story is still one untested paragraph.

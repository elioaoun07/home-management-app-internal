...
### Visibility & access (RLS **and** app-layer — both must allow the row)
> **CORRECTED 2026-08-04.** This section previously stated "No RLS is enabled on `trips`/`trip_places`/`trip_packing_items`/`trip_side_effects`/`trip_documents`; access is enforced entirely in the API routes." **That was false and it caused a live bug**: `pg_class.relrowsecurity` shows RLS **enabled** on `trips`, `trip_places` and `trip_packing_items`, with own-user-only policies. Because the docs said RLS didn't exist, no household-aware policy was ever written, and partners' household trips were silently filtered out of `GET /api/trips` — correct app logic, zero rows, no error. Fixed by `migrations/2026-08-04_trips-household-rls.sql`. Doctrine §7.3: verify RLS against the live DB, never against this file.

Current state: RLS is enabled on `trips`, `trip_places`, `trip_packing_items`, `trip_packing_category` and `trip_documents` (the last two were RLS-*off* until 2026-08-04 — an open read/write hole through PostgREST — and are now closed). Access is enforced at **both** layers, and both must allow the row:
- **DB layer** — the own-user policies, OR'd with the permissive household policies added 2026-08-04. The partner predicate lives in two SECURITY DEFINER helpers: `is_household_partner(uuid)` (household_links lookup, RLS-safe) and `trip_is_accessible(uuid)` (owner-or-household-partner, used by every child table policy). Using a function rather than an inline `EXISTS (SELECT 1 FROM trips …)` is Hard Rule #20's preferred option, not a violation of it.
- **App layer** — `getAccessibleTrip()` (`src/lib/tripAccess.ts`), mirroring the `is_public` account pattern (`getAccessibleAccount()` in `src/lib/accountAccess.ts`):
...
`trip_packing_category` *(added 2026-08-04)*: collaborative, per-trip category lookup rows. `trip_packing_items.category_id` references a category row and is the sole category field for packing items; category CRUD lives at `GET/POST /api/trips/[id]/packing/categories` and `PATCH/DELETE /api/trips/[id]/packing/categories/[categoryId]`. Every route uses `getAccessibleTrip()`. It shipped with RLS disabled (following the since-disproven "trips family has no RLS" note above, which left it readable and writable by any authenticated user through PostgREST); RLS was enabled with a `trip_is_accessible(trip_id)` policy on 2026-08-04. Categories are keyed per **trip**, not per user — `UNIQUE (trip_id, lower(name))` — so both household members share one set and `user_id` records only who created the row. Never insert a per-user duplicate.
...
### `trip_packing_items`
Packing list. `is_packed` toggled optimistically. Optional `catalogue_item_id` links to a Catalogue `documents`-type item (passport, visa) — accepted by the API but **no picker UI exists yet** (junction work, deferred). `assigned_to uuid` *(added 2026-08-03)* — optional household-member assignment ("Mine"/partner's name filter chips), person-absolute color identity per Hard Rule #14. `position` supports drag-reorder (`POST .../packing/reorder`) for items with no natural sort order. `deleted_at timestamptz` *(added 2026-08-04, migration pending)* — soft delete. `GET .../packing` and `get_trip_bundle()` both filter `deleted_at IS NULL`; `DELETE .../packing/[itemId]` sets it instead of hard-deleting so `POST .../packing/[itemId]/restore` can bring a row back with its exact `quantity`/`packed_quantity`/`is_packed`/`position`/`category_id` intact. `GET .../packing/deleted` lists soft-deleted rows for one trip — backs an in-context "Deleted items" sheet in `TripPackingList.tsx`, distinct from the app-wide Recycle Bin module (nothing here is wired into `src/features/recycle-bin/`).

### `trip_packing_checkpoints` *(added 2026-08-04, migration pending)*
One collaboratively-writable row per trip (`UNIQUE (trip_id)`), upserted by `POST .../packing/checkpoint`: `snapshot jsonb` = `[{id, packed_quantity, is_packed}]` for that trip's non-deleted packing items at last quick-save, `created_at` = when it was saved. `POST .../packing/checkpoint/revert` re-applies the snapshot onto `trip_packing_items`. Read via `GET .../packing/checkpoint` (returns `{created_at}` only — the client doesn't need the snapshot body). **Deliberately not a column on `trips`**: per `migrations/db-state.json` (generated 2026-08-04T10:06:44Z), `trips` has only an owner-only `ALL` policy (`trips_owner`) and a partner `SELECT`-only policy — no partner `UPDATE` — so a `trips.packing_checkpoint` column would have silently no-op'd for the non-owner partner (Hard Rule #27's failure class). Its own child table with a `trip_is_accessible()` policy (mirrors `trip_packing_category`) keeps it collaboratively writable like the rest of packing. `POST .../packing/checkpoint` also accepts `{ restore: { snapshot, created_at } }` to re-upsert a prior snapshot verbatim — that's how the save toast's Undo works without leaving the wrong snapshot as the source of truth for a later revert.
...
7. **Visibility is enforced at BOTH layers — RLS *and* app-layer** *(corrected 2026-08-04; this trap previously said the opposite and was itself the cause of the partner-invisible-trips bug)*: every route must still call `getAccessibleTrip()` (or filter to `user_id = auth uid` for owner-only routes), **and** the row must survive RLS. The failure mode to watch for is the silent one: correct route logic returning zero rows because no policy grants the partner access. When adding a new trip child table, give it a `trip_is_accessible(trip_id)` policy in the same migration — do not repeat the "trips has no RLS, so children don't need policies" assumption. Verify with `select relname, relrowsecurity from pg_class`, never from this doc.

...
- **Per-message color tags + color filter (HUB-11, shipped 2026-08-06)**: any message can be tagged with a color from a fixed 8-swatch preset (`hub_messages.color`, `src/features/hub/messageColors.ts`) — no color has a fixed meaning, the household decides (e.g. one color per person, or transfer vs expense). Set the "active compose color" via the palette button next to the input (sticky per-thread, so a batch of messages can be logged under one color without reselecting); long-press an existing message → the action menu's color row to recolor it. A header **Filter** button narrows the thread to one color at a time. When a color filter is active, "Select all" in Multi-add only auto-checks messages matching that color (`getSelectAllEligibleIds` in `ThreadConversation`, `HubPage.tsx`) — so a noisy budget thread can be swept color-by-color (e.g. filter to "mine", select all, add as transactions; repeat for "partner's").

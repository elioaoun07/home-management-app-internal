...
- [ ] **WP-T5 · Doors + polish · 0.5** — prereq: T2–T4. Deep-links from every tile (faceRegistry routes + card-type → module route map); staleness stamps; empty-state ("Nothing needs you"). Skills: `ui-guardrails`. → *Gate:* every visible element navigates somewhere; Atlas entry updated (Hard Rule 23).

...
- 🔴 **SDK cumulative dollars are added as fresh turn charges.** `scripts/delivery/drivers/claude.mjs:575–596,1319` returns the reused query's cumulative reading; `run-session.mjs:1059–1064,1833–1863` adds it. Aug6 segment metadata confirms the unit mismatch ([Orchestration §3](<../Command Center/Command Center — ASTRA Orchestration.md>)); billing basis remains owner-UNVERIFIED. DLV-87/88 · ASTRA-DLV-1.
- 🔴 **A successful retry loses earlier failed-attempt spend; REVIEW can start UAT after crossing its cap.** The failed bank at `run-session.mjs:1772–1793` is absent from the success return (`:1853–1863`); REVIEW's usage at `:4115` is neither persisted nor budget-checked before UAT (`:4214,4620,4716`). DLV-95's parse repair did not close these boundaries. DLV-87 · ASTRA-DLV-2; unfinished-attempt reservation remains held DLV-68 work.
- 🟠 **INSTANT accepts surplus same-file changes and unsupported acceptance claims.** `instant.mjs:256,259` checks containment; `:324–328` promotes every AC, while `acceptance.mjs:165–166` accepts a changed/existing filename. The study's pure fixtures accept an extra edit and label unrelated offline/device ACs met. ASTRA-DLV-5 is a conditional DLV-93 validation alternative, not a new immediate slot.
- 🟠 **Missing or unreadable review meaning can advance to UAT.** `run-session.mjs:4125–4135` drops malformed findings with a diagnostic; only `verdict === "BLOCK"` stops progression (`:4172`). ASTRA-DLV-6 competes with ASTRA-DLV-5 for DLV-93's single validation remedy.
- 🟠 **Processed token traffic drives false context pressure.** `usage.mjs:137` and `run-session.mjs:1752,629` feed aggregate request traffic into rotation; `context-policy.mjs:65–76` treats it as resident context. The accepted metadata study separates 2,020,276 total input-side tokens from 131,439 maximum/latest request context. DLV-69 · ASTRA-DLV-3, held under the freeze.
- 🟠 **Current finish evidence can contradict session state and repeat completed work.** The inspected Aug22 HUB-1 state is ACCEPTED but its older finish manifest is BLOCKED with 0/6 ACs; `finish-package.mjs:83–85` also lists completed plan steps as remaining after `build:null` (`run-session.mjs:3785`). Finish writes are best effort (`:910–947`). [Command Center CC-3](<../Command Center/Command Center — ASTRA Packets.md>) owns consistency; no historical artifact or shipped record is rewritten here.
- 🟠 **Phone command timeout does not prove the action failed.** `src/features/pm-live/usePmLive.ts:147–161` installs its waiter after insert and lacks status-query recovery. `scripts/pm/bridge.mjs:1123–1157` claims before execution and ignores final-update errors; an interrupted claim is not drained again. CC-1 covers same-ID acknowledgment reconciliation; bridge crash receipts remain held. Never auto-resubmit uncertain grants.
- 🟠 **The phone hydrates an unscoped cache and merges complete reads.** `src/features/pm-live/cache.ts:13–29`, `usePmLive.ts:35–50`, `store.ts:73–107` permit prior-owner or deleted rows to remain in client state. This is a verified client-state defect, not an assertion about live RLS. CC-2 owns owner/schema-bound hydration and authoritative replacement.
- 🟡 **The mobile-approval migration runbook is missing, so its application status is not established.** The DLV-77 filename is absent; source gate checks at `scripts/pm/bridge.mjs:785–827` cannot establish the live CHECK. Owner-provided current constraints and the applied ledger must settle it; do not reconstruct guessed SQL or infer “not applied” from absence.
...
- 🟡 **Durable attempt identity and ownership expiry are still missing; heartbeat itself exists.** `run-session.mjs:2012` allocates from the not-yet-persisted counter; the inspected Aug22 transcript has 17 sealed rows but 11 unique IDs. A five-second heartbeat/PID probe already exists (`:4722–4754,4864`); build locks still derive from state including ACCEPTED (`server-routes.mjs:83,464–475`). DLV-68/91 target reservation, progress and lease/recovery gaps, not another timer; held statuses remain.
...
- 🟡 **The active forecast has a phase-total/per-traversal mismatch.** ASTRA corrects the earlier `PHASE_BASELINE_TOKENS` attribution: `STATIC_USAGE_BY_TIER` is legacy; the headline uses `STATIC_PHASE_USAGE_ECONOMY` / `forecastByPhase` (`recommendation.mjs:78,115–128,190–194,620`). History supplies whole-phase totals (`server-routes.mjs:401–428`), then the forecast multiplies them again. The inspected history has fewer than three eligible samples per tier, so the faulty measured branch is latent, not today's active source. DLV-86 / ASTRA-DLV-8 follows clean receipt units; no calibration platform is authorized.

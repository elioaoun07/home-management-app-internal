...
- [ ] **BUD-32** *(packet **M-01** of the [ERA Top Layer — Master Plan](<../ERA Top Layer — Master Plan (2026-09-02).md>) — must land before BUD-39/BUD-36's owner-run day)* Free the statement fingerprint when an imported transaction is deleted — `DELETE /api/transactions/[id]` sets `deleted_at` but leaves `statement_hash`, so the partial unique index still holds it: re-importing reports `skipped_duplicate`, the row never returns, and reconcile (which filters `deleted_at IS NULL`) shows it as unmatched so Commit silently reports 0 created. Either null the hash on soft-delete, or make reconcile see soft-deleted hash-bearing rows and say "deleted — restore instead". → `src/app/api/transactions/[id]/route.ts` + `src/app/api/statement-import/reconcile/route.ts` _(friction - S)_

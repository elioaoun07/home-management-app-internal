...
- 🔴 **Hard Rule 6 (the `safeFetch()` choke point) is breached at ~98 client call sites** — raw `await fetch("/api/…", { method: "POST" })` in `src/components/`, `src/features/`, `src/hooks/`, `src/contexts/`. Worst concentrations: `hub/ShoppingListView.tsx` (13), `useNotifications.ts` (8), `expense/CategoryManagerDialog.tsx` (8), `features/hub/hooks.ts` (7), `settings/CategoryManagement.tsx` (7). Every one of these bypasses the pre-flight online check, the 3 s timeout, and `markOffline()` — so on those paths the offline queue and the false-offline indicator simply do not engage. This is a Doctrine §4 standing decision ("all mutations through `safeFetch()`") failing silently, and it is invisible to typecheck. Measured 2026-08-01. Server-side raw `fetch` (6 sites in `src/app/api` + `src/lib`) is legitimate — those call external APIs.
- 🔴 **Hard Rule 20's forbidden RLS pattern is present on all five tables the rule names by name** — `migrations/schema.sql:1512-1516` defines `event_details_via_parent`, `item_alerts_via_parent`, `item_recurrence_rules_via_parent`, `item_subtasks_via_parent`, `reminder_details_via_parent`, each an `EXISTS (SELECT 1 FROM items i WHERE i.id = child.item_id AND …)` with a *second* nested `EXISTS` over `household_links` inside it. The rule says never do this; the shipped schema does it fivefold. Mitigated in practice only because the hot read path goes through `get_schedule_bundle` (SECURITY DEFINER, bypasses per-table RLS) — but any direct PostgREST read of those child tables pays the full nested-join cost per row. Needs a decision: rewrite the rule to "never let a hot path traverse these; route through the bundle RPC", or denormalize `user_id` onto the child tables as the rule's own second option prescribes. Measured 2026-08-01 against schema.sql; live DB not verified (Hard Rule 26).

...
- 🔴 **Same-snapshot scanner parity does not protect a stale mutation's target.** Toggle/Ship/Discard select an ordinal without complete expected-item identity (`scripts/pm/mutations.mjs:24`; `scripts/pm/archive.mjs:237–247`); the accepted pure fixture checks a prepended open row instead of the intended one. `scripts/pm/shared/tasks.mjs:58` also keys postponement by ordinal, contradicting the text-keyed standing description and transferring hidden state after reorder. ASTRA-R-1 refines the historical open R7 work, now R51; shipped consolidation R7 remains unchanged.
- 🔴 **Desktop Undo can overwrite a later edit.** `scripts/pm/archive.mjs:384–391` restores pre-images without checking current post-operation identity; `pm-server.mjs:309` exposes that path. The bridge already checks `afterHash` (`bridge.mjs:619`). ASTRA-R-1 requires checking every affected file before any restore.
- 🟠 **Historical ID reuse made task identity ambiguous.** At the study cutoff, R49 named shipped archive automation and queued CI; R7 named shipped consolidation and queued SSE/drift verification. Phase-5 document reconciliation retains shipped IDs, folds CI into HUB-37/E-01 and renames the open SSE/refinement work R51. The underlying guard still checks current-checklist uniqueness, not lifetime uniqueness (`scripts/pm/lint.mjs:126`); no enforcement code was changed.
- 🟡 **CRLF frontmatter is parsed as missing metadata.** `scripts/pm/shared/frontmatter.mjs:14` returned `{}` for the study's in-memory CRLF fixture and produced E1 status/date errors. Preserve LF for document landing; any parser behavior fix is separate from R36's checked-JSDoc contract.
- 🟡 **Annotations alone do not enforce the shared JS contracts.** `tsconfig.json:5` allows JS without `checkJs`; R36 must actually execute the scoped checker and its deliberate-error proof. ASTRA-R-4 is held by the existing product-first admission rule.
...
- 🟡 **The bespoke JS surface is larger than the historical 7,150-line estimate.** Accepted source recount: 10,602 JS/JSX/MJS lines under `scripts/pm/`, including 4,534 legacy-client lines, plus 611 in `pm-server.mjs`; 13 feature directories and 11 PM UI tests. Source reuse and types do not replace runtime proof. R6 retirement and R36 checked contracts remain bounded, held behind their existing gates.
- 🟡 **Cached shell/data compatibility is unverified despite existing offline honesty.** `scripts/pm/assets/sw.js:39,53` supplies offline headers and `scripts/pm/src/app/store.js:77,99,132` blocks offline writes, but fixed `pm-offline-v1` and independently cached shell/data do not establish build/schema compatibility (`scripts/pm/assets/sw.js:8–9`). R37 / ASTRA-R-2 owns that missing contract; hosted `/pm` and owner-bound phone cache remain separate paths.
...
- 🔴 **Raw client mutations still bypass the `safeFetch()` choke point; the ~98-site count is historical (2026-08-01).** The earlier 3 s description is superseded: `src/lib/safeFetch.ts` uses an 8 s default and a health probe before treating an endpoint timeout as offline. A helper call does not itself provide a durable queue. R44/R48 must remeasure scope and preserve the verified timeout/connectivity distinction; ASTRA-R-5 later consumes an actual detector, not a prose rule.
- ⚪ **The earlier five-policy allegation was sourced from `schema.sql` and is not admissible live-RLS evidence.** The committed Aug4 `migrations/db-state.json` is historical and stale; current child-table policies/functions remain **UNVERIFIED** until the owner supplies fresh output or Hard Rule 27's two untruncated queries. R43 must begin with that evidence, not a policy rewrite or a rule amendment based on the prior schema quotation.

...
| **RRULE recurring** | 🟢 Core | `item_recurrence_rules.rrule` expanded against `start_anchor` (the DTSTART). Wall-clock DST adjustment. Bi-weekly detect + phase-flip. Per-occurrence actions in `item_occurrence_actions`; exceptions in `item_recurrence_exceptions`. **Math untested.** | Unit-test occurrence expansion + exception/skip logic. |

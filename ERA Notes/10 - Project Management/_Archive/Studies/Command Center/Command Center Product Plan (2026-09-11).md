...
status: proposed
owner: Elio
evidence_cutoff: "2026-09-11 working tree over 8d952332b0d7917369ce074730cfe830a5c37a97; source review and local fixtures, no live delivery or device acceptance"
---

# Command Center — Product Direction and Implementation Plan

This is the requested plan for Claude Code to execute subsequently. No application implementation, provider run, production operation or policy change was performed. It concerns `pnpm pm`, Delivery v2 and the phone experience for that product. The household application's other modules are delivery targets, not redesign targets.

The recommendations below are concrete proposed decisions. The current campaign checklists remain the only executable backlog. Adoption should reconcile this document into the existing PM Tooling and Delivery campaigns and [Delivery V2](<../Plans/Delivery V2.md>); it should not create another campaign or competing checklist.
...
| Recommendation/finding | Canonical owner/disposition on adoption |
|---|---|
| Keep the React reboot and finish its work journey | PM Tooling R57/R58 foundation; refine R6/R42 acceptance where applicable. New Board/Sprints outcome IDs only after checking existing lifetime IDs. |
| ID/source/witness and guarded mutation repairs | Extend R51 where scope overlaps; add the specific parser/source outcomes to PM Tooling once. Initial v2 selection witness belongs DLV-97 with cross-reference. |
| V2 isolation, qualification loading, checker boundary | DLV-96; retain the existing refusal battery. |
| Actual dispatch, approvals, per-run executor/model/effort and resources | DLV-97 / DEC-14; PM presentation under R55. DLV-99 owns the admitted pilot, DLV-100 its failure/recovery matrix, and DLV-101 interrupted DEEP continuation; preserve their distinct acceptance. |
| Unknown cost and provenance | DLV-87/88; forecast work DLV-86 only where it remains needed. V1 repair scope does not silently become a full v2 redesign. |
| Phone command/cache recovery | New single Delivery recovery outcome on adoption; R55 for v2 presentation. Preserve DLV-77's DB command-constraint verification and R42's phone/PWA acceptance as separate obligations; they do not already own this implementation. R37 applies where source/owner provenance scope overlaps. |
| Phone as a required product milestone | Proposed replacement of DEC-15's optional-mobile direction; managed hosting stays optional. |
| Owner-triggered protected Apply | Proposed DEC-16 change; keep no autonomous Git/publication or production DB authority. |
| V2 streamlined approvals | Define v2 policy under DEC-14. DEC-21 continues to govern V1 only; do not quietly change V1 gates. |
| Native baseline / useful remote trial | DLV-98 / DLV-102; measure total owner work and usefulness, not agent count. |
| Supported V1 retirement | DLV-103 after DLV-102, with shared imports/history and recovery paths preserved. |
| Sprints, capacity, reliable parallel delivery and metric definitions | Proposed outcomes in PM Tooling/Delivery, not existing implemented features; allocate one owner per outcome on adoption. |
...
Retire this proposal after adoption: move the surviving specification into the accepted plan/books, map each phase to canonical outcomes, and archive the assessment with this evidence cutoff intact. If the direction is declined, keep it as an unadopted research option rather than another active queue.

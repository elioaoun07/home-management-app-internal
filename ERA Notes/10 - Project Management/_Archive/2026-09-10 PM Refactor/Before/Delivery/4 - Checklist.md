...
- [ ] **DLV-86** (ASTRA-DLV-8) Correct the active phase-based forecast units after ASTRA-DLV-1/2 repair receipt provenance; distinguish phase totals from per-traversal priors and exclude ambiguous history. Measured effort calibration remains gated on sufficient clean samples; do not retune the obsolete aggregate constant. → [Execution sheet](<ASTRA/Delivery — ASTRA Packets.md>) _(blocker - M)_
...
- [ ] **DLV-89** (ASTRA-CC-C04; HELD for owner decision) Resolve the proposed interaction-cost experiment for DLV-92 against the three-gate non-negotiable. Existing qualifying INSTANT gate compression remains valid; no general typed-approval, budget or gate waiver is authorized by this study. → [Governance evidence](<../Command Center/Command Center — ASTRA Architecture.md>) _(friction - M)_
- [ ] **DLV-90** (ASTRA-DLV-4) A targeted run selecting zero tests records NOT_TESTED and cannot satisfy INSTANT validation, regardless of process exit code. → [Execution sheet](<ASTRA/Delivery — ASTRA Packets.md>) _(friction - S)_
...
- [ ] **DLV-77** (ASTRA-CC-C13) Owner supplies the current pm_commands CHECK definition and resolves the missing historical migration reference; verify approve/accept eligibility against the bridge contract before any owner-applied repair. The absent file does not prove current DB failure; agents make no DB calls. → [ASTRA evidence](<ASTRA/Delivery — ASTRA Book.md>) _(blocker - S)_
...
**ASTRA wave 1 (2026-09-06)** *(study: [ASTRA Book](<ASTRA/Delivery — ASTRA Book.md>); sheets: [ASTRA Packets](<ASTRA/Delivery — ASTRA Packets.md>))*

ASTRA-DLV-1/2 refine DLV-87/88; ASTRA-DLV-8 refines DLV-86; ASTRA-DLV-4 refines DLV-90. These are existing prerequisites, not extra workstreams. DLV-69 remains held for ASTRA-DLV-3/7 until freeze eligibility. ASTRA-DLV-5/6 and ASTRA-R-5 compete for one DLV-93 validation slot; ASTRA-CC-1/2/3 and ASTRA-R-1 compete for one reliability slot. No slot is selected by this study, no two-completion gate is waived, and no new Delivery ticket is added.

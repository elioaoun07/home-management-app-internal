...
- ✅ **On-device background removal fixed — two-layer WASM failure** (2026-07-19): real-phone D2 test surfaced `removeGarmentBackground` failing every retry. **Layer 1:** onnxruntime-web's WASM backend does a nested dynamic `import()` of a `blob:` module, governed by CSP `script-src` (not `worker-src`) — `next.config.ts` script-src was `'self' 'unsafe-inline'` only, so it was silently blocked. Fixed by adding `blob:` + `'wasm-unsafe-eval'` to `script-src`. **Layer 2** (surfaced after Layer 1 fix): `@imgly/background-removal` unconditionally sets `ort.env.wasm.numThreads = navigator.hardwareConcurrency`, forcing the multi-threaded WASM build, which requires `SharedArrayBuffer` (only available cross-origin-isolated via COOP+COEP headers — not set site-wide here, deliberately, since `COOP: same-origin` risks breaking the Google Calendar OAuth popup's `window.opener`). Fixed by shadowing `navigator.hardwareConcurrency` to `1` for the duration of the cutout call in `backgroundRemoval.ts` (`withSingleThreadedWasm`), forcing ORT onto the single-threaded path — slower per-cutout, zero blast radius elsewhere. Diagnostic side-fix kept: `AddGarmentSheet.tsx` surfaces the real `err.message` in the amber banner if cutout ever fails again. Needs a real-phone retest to confirm the cutout now completes end-to-end.

...
- ✅ **On-device background removal CSP block fixed** (2026-07-19): real-phone D2 test surfaced `removeGarmentBackground` throwing `"no available backend found... Failed to fetch dynamically imported module: blob:..."` on every retry. Root cause: onnxruntime-web (used by `@imgly/background-removal`) spins up its WASM backend via a nested dynamic `import()` of a `blob:` module — that call is governed by CSP `script-src`, not `worker-src`, and `script-src` in `next.config.ts` was `'self' 'unsafe-inline'` only (no `blob:`), so the browser silently blocked it every time regardless of photo/network/device. Fix: added `blob:` and `'wasm-unsafe-eval'` to `script-src`. Diagnostic side-fix kept: `AddGarmentSheet.tsx` no longer swallows the cutout error — the amber banner shows the real `err.message` if it ever recurs. Needs a real-phone retest to confirm the cutout now completes end-to-end.

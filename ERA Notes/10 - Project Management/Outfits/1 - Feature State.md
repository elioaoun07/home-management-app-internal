...
- 🟠 **On-device background removal failing consistently** (2026-07-19): real-phone D2 test surfaced `removeGarmentBackground` (`@imgly/background-removal`, default CDN `staticimgly.com`) throwing on every retry of the same photo — not a transient network blip. CDN itself verified reachable (HTTP 200) from a dev machine, so root cause is device/network-side (ad-blocker/DNS filter blocking the model CDN, private-mode cache-storage eviction of the ~40-80MB model, or low-memory WASM OOM) — not yet confirmed which. Diagnostic-only fix shipped: `AddGarmentSheet.tsx` no longer swallows the error — the amber "Couldn't cut this one out" banner now shows the real `err.message` so the actual cause can be identified from the user's device. Module still fully usable via "Keep original" fallback in the meantime. _(blocker for cutout quality, not for shipping)_

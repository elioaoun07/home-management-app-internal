...
| Intent architecture | 7      | now **fixture-covered + graceful fallback** — a misrecognized intent clarifies instead of firing a wrong action (HUB-1, 2026-08-22)                                                               |
| Test protection     | 4 (+2) | **two test files now**: `chatNotificationPolicy.test.ts` (2 green) + `rootIntentRouter.test.ts` (39 green, table-driven, HUB-1). Intent routing — the money-adjacent path — is no longer untested |
...
| AI Assistant (ERA)             | 🟡   | the flagship: intent router, faces, widgets, wake listener, budget submit, household context; `src/features/era/` is the largest feature dir (28 files). Intent router now fixture-covered with a graceful `clarify` fallback (`intents/rootIntentRouter.test.ts`, HUB-1 2026-08-22). |

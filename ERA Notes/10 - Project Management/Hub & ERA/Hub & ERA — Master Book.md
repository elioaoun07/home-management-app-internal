...
- 🟡 **Voice conversation speaks in a different voice from ERA — and its reminders don't save.** `voice-conversation/speechTemplates.ts` is a parallel, un-pooled reply surface ("Reminder set: {title}.") that never got the HUB-15 treatment, and `intentClassifier.extractReminderTitle` strips the date with `/\b(in|at|on|tomorrow|today|tonight|next)\b.*$/` and discards it. Worse, `HubPage.onSetReminder` posts a chat message and opens `AddReminderFromMessageModal` defaulted to *today, next hour* — so "Reminder set" is spoken before anything is set, and the parsed time is lost. Should route through `resolveDraftReminder` + the shared pools. Surfaced 2026-08-22 (HUB-15).

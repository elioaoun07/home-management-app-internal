...
- [ ] 2026-08-19 — SECURITY (found by the Supabase advisor while working statement import, not acted on): RLS is **disabled** on `public.guest_drinks` and `public.item_prerequisites`, so both are readable/writable with the anon key. Needs an owner decision because enabling RLS without policies blocks all access — the guest-drinks table is deliberately reached by unauthenticated guests via `/g/[tag]`, so it needs a policy shaped around the tag flow, not a blanket lock. Suggested first step: `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` plus explicit policies, verified against `migrations/db-state.json` per Hard Rule #27.

...
- **Components**: `src/components/pm-live/` — `PmLiveApp.tsx` (shell), `shell/` (TopBar, SideNav, BottomNav, UndoStrip, navItems), `views/` (Overview, Board, Delivery, Usage, Campaigns), `session/` (`SessionDetailView.tsx`, `SegmentedPanes.tsx`, `panes.tsx`), `board/` (BoardToolbar, TaskRow), `widgets/` (WidgetCard, StatTile, SeverityBar, BarList, TrendArea, ChartTooltip), `Sheet.tsx`, `CaptureSheet.tsx`, `LaunchSheet.tsx`, `TaskDetailSheet.tsx`, `BridgeStatusChip.tsx`.
...
| **Delivery** | Fleet control — session cards with budget burn, gate hints, the agent's actual question text, and the pause/stop-turn/resume/guidance/cancel actions. Tapping a card opens the **session detail**. |
| **Session detail** (`?session=<id>`) | One session in full, behind a sticky segmented pill bar over snap-scrolling panes: **Questions** (the gate question plus the whole Q&A ledger, each answerable inline), **Conversation** (the last ~40 turns, role-coloured, with an excerpt on the most recent 12), **Artifacts** (spec / plan / finish summary / remaining work / recovery, excerpted), **Cost** (envelope burn, spend by phase and by model, context-window occupancy). |

...
- **Sub-components**:
  ...
  - `src/components/statement-import/GroupSheet.tsx` — the drawer where a merchant is decided: category grid, plus per-row category/date/skip and the date shift behind a collapsed disclosure
...
- Filters (Review / Matched / Skipped) are a full-width segmented control over a progress bar, and mirror the reconciliation buckets.
